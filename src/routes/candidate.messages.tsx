...
type Party = { id: string; full_name: string | null; email: string; company_name: string | null };
...
function Messages() {
  ...
  const threadsQ = useQuery({
    enabled: !!user?.id,
    queryKey: ["candidate", "threads", user?.id],
    queryFn: async () => {
  ...
  const partiesQ = useQuery({
    ...
    queryKey: ["parties", partyIds.join(",")],
    queryFn: async () => {
      const { data, error } = await supabase.from("profiles").select("id,full_name,email,company_name").in("id", partyIds);
      if (error) throw error;
  ...
  const send = useMutation({
    ...
    onSuccess: () => {
      setDraft("");
      qc.invalidateQueries({ queryKey: ["messages", active] });
      qc.invalidateQueries({ queryKey: ["candidate", "threads"] });
    },
  ...
  const partnerLabel = (p: Party | null | undefined) => p ? (p.company_name ?? p.full_name ?? p.email) : "Contact";

  return (
    <>
      <PageHeader title="Messages" description="Talk directly with hiring teams and universities." />
      {threadsQ.isLoading ? (
        <div className="flex justify-center py-12"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div>
      ) : threads.length === 0 ? (
        <div className="rounded-xl border border-dashed border-border bg-card p-12 text-center text-sm text-muted-foreground">
          No conversations yet. Hiring teams will reach out here after reviewing your submissions.
        </div>
      ) : (
          ...
          <div className="border-b border-border md:border-b-0 md:border-r overflow-y-auto max-h-[600px]">
            {threads.map((t) => {
              ...
              return (
                ...
                >
                  <Avatar className="h-9 w-9"><AvatarFallback>{(partnerLabel(p ?? null))[0]?.toUpperCase() ?? "?"}</AvatarFallback></Avatar>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between">
                      <div className="truncate text-sm font-medium">{partnerLabel(p ?? null)}</div>
                      <div className="text-[10px] text-muted-foreground">{formatDistanceToNow(new Date(t.last_message_at), { addSuffix: true })}</div>
                    </div>
                    {t.subject && <div className="mt-0.5 truncate text-xs text-muted-foreground">{t.subject}</div>}
                  </div>
                </button>
              );
            })}
            ...
            {activeThread ? (
              <>
                <div className="border-b border-border p-4">
                  <div className="font-medium">{partnerLabel(partner ?? null)}</div>
